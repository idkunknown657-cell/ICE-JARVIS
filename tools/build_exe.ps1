# Builds the distributable JARVIS.exe (and the update manifest) on Windows.
#
#   powershell -ExecutionPolicy Bypass -File tools\build_exe.ps1
#
# Steps:
#   1. installs requirements-dev.txt (includes PyInstaller + the app deps)
#   2. runs PyInstaller against JARVIS.spec  ->  dist\JARVIS.exe
...
Write-Host "==> Building JARVIS.exe (PyInstaller)..."
python -m PyInstaller --noconfirm --clean "$Root\JARVIS.spec"
