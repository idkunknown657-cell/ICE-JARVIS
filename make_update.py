"""
make_update.py — package the current dist/ICE build into a pushable update.

    python make_update.py --url https://your-host/ice/ICE_update.zip
    python make_update.py --version 2026.10.1 --notes "Softer voice, faster replies"

What it produces (next to dist/):
    ICE_update.zip        the whole app, zip-root = dist/ICE contents
...
HOW YOU PUSH AN UPDATE ("from here")
  ...
  3. Pack:       python make_update.py --url https://<your-host>/.../ICE_update.zip
...
def main() -> None:
    ...
    if not (DIST_APP / "ICE.exe").exists():
        sys.exit("✗ dist/ICE/ICE.exe not found — run build_exe.py first")
    ...
    zip_path = out_dir / "ICE_update.zip"
